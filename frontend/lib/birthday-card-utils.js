...
export async function sendBirthdayCardViaWhatsApp(
  ...
  try {
    ...
    if (
      ...
      try {
        await navigator.share({
          ...
          text: `🎉 كل عام وبطلنا الغالي *${player.name}* بألف خير بمناسبة عيد ميلاده المبارك (${turningAge} سنة)! 🎂🥋`,
        });
        if (onNotice)
    ...
    // 3. Open WhatsApp (with cleanPhone or without)
    ...
    if (onNotice) {
      onNotice(
        copied
          ? "✓ تم نسخ كارت عيد الميلاد للحافظة وفتح واتساب! الصق الصورة (Ctrl+V أو لصق) ثم اضغط إرسال."
          : "جاري فتح واتساب لإرسال تهنئة عيد الميلاد..."
      );
    }
  } catch (err) {
    ...
    if (onProgress) setTimeout(() => onProgress(false), 2000);
  }
}
