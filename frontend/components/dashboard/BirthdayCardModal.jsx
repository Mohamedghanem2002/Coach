...
import { X, Download, Send, ClipboardCopy } from "lucide-react";
import {
...
export default function BirthdayCardModal({
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      setCopied(true);
      setNotice("✓ تم نسخ كارت عيد الميلاد للحافظة بنجاح!");
  ...
  return (
    ...
    >
      <div className="relative max-h-[94vh] w-full max-w-sm sm:max-w-md overflow-y-auto rounded-3xl border border-amber-300/40 bg-gradient-to-b from-slate-900 to-slate-950 p-4 sm:p-5 text-white shadow-2xl">
        ...
        {/* Card Image Display */}
        <div className="relative mb-3 flex items-center justify-center overflow-hidden rounded-2xl border border-amber-400/30 bg-slate-950/80 p-2 min-h-[300px]">
          {isLoading ? (
            <div className="flex flex-col items-center gap-2.5 py-12">
              ...
              <span className="font-cairo text-xs font-bold text-amber-200">
                جاري تصميم كارت عيد الميلاد الفخم...
              </span>
            </div>
          ) : dataUrl ? (
            <img
              ...
              className="max-h-[55vh] w-auto rounded-xl shadow-lg object-contain"
            />
          ) : (
          ...
          >
            {isSending ? (
              ...
              <>
                ...
                <span>إرسال كارت التهنئة عبر واتساب 📲</span>
              </>
            )}
          </button>
